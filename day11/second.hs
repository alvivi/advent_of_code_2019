...
runProgram source =
  ...
  in
    step (0, 0) DUp 0 0 [] (Map.singleton (0,0) 1) source
...
main =
  let
    ...
    toChar i = if i == 1 then '#' else ' '
  in do
    contents <- T.pack <$> getContents
    let panel = runProgram $ buildSource $ parse contents
    let width = maximum $ map fst $ Map.keys panel
    let height = maximum $ map fst $ Map.keys panel
    let row y = map (\x -> toChar $ fromMaybe 0 $ Map.lookup (x, y) panel) [0..(width - 1)]
    mapM_ (putStrLn . row) [0..(height - 1)]
